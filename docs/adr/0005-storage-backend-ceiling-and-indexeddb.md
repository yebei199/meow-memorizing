...
accepted — 去掉 `unlimitedStorage`、修掉写满时的静默中断；**不换存储后端**，
IndexedDB 的触发条件记录在下方
...
- IndexedDB 那 0.06ms 是**等事务真正提交**（`transaction.oncomplete`）测出来的，不是只等
  `put()` 请求返回的假象；两者都远低于 1ms，早先记的"0ms"只是取整所致。它与
  `chrome.storage.local` 的 24ms 差两个数量级，但在一次 mouseup 上人无法感知——**这不是
  换后端的理由**，容量才是。
...
而 `chrome.storage` 有 WXT 的 `storage` 封装现成可用；为一个尚未触及的天花板换掉够用的
方案，不值得。

要说清一个**明知的代价**：现在恰恰是最便宜的时机——[ADR-0004] 的迁移只在 `dev` 上，
已发布的 v1.1.1 词库仍在 `sync` 区，此刻改目的地，用户仍然只迁一次。选择推迟，就意味着
触发条件到来时那将是**第二次**动用户真实词库。这个取舍是清醒做出的：现状对 5 万词的
设计目标够用，而 IndexedDB 多出的容量当下无人需要，不为尚未存在的需求提前付复杂度。
...
静默中断——这正是 [ADR-0004] 那次 8KB 配额故障的**表现放大器**：它把"存储满了"这种局部
的、可恢复的错误，放大成"整个插件坏了"。天花板从 8KB 换到 10MB，失败模式却没换，撞上时
症状会与那次一模一样。

**决定**：记录单词是尽力而为。翻译根本不依赖写入，所以写入失败只让「保存」失败：
`AddButton` 兜住 `addQueriedWord` 的异常并记日志，翻译卡片照常弹出。卡片无需改动——
它的 `isSavedSelection`（见 `PanelComponents.tsx`）本就会在词条不存在时诚实显示「未收录 /
未加入词库，页面不会高亮该词」，而不是谎称已保存。回归见
`tests/e2e/vocabulary-full.spec.ts`，它用「压舱物」把 local 区顶到离上限 60 字节来触发真实
的 `kQuotaBytes`，比灌 6.6 万个真词快得多。

词汇书页面的删除/停用路径（`HoverTooltip` 的 `setTimeout` 回调）有同样的无兜底形状，但
爆炸半径小得多：它们已经先乐观地更新了 UI，写入失败只造成高亮与存储短暂不一致，下一次
整页扫描即自愈。本次不动。
