...
**决定**：词库改存 `local` 区，且**一词一条**（`local:word:<单词>`），并声明
`unlimitedStorage` 权限。
...
- **unlimitedStorage**：5 万词约 7MB，占 local 默认 10MB 上限的 70%，留作余量。

  > **后续修正（见 [ADR-0005]）**：该权限对 5 万词的设计目标**零收益**——实测不带它也装得
  > 下，余下约 3MB 约合再放 2 万词。它买的是 7 万词以上的空间，不是 5 万词本身。真到那时
  > IndexedDB（10GB，无需权限）才是正解。是否去掉见 [ADR-0005] 的结论。
...
- **IndexedDB**：容量与查询能力都更强，当前瓶颈不在此，先不引入。
  ...
  > **后续修正（见 [ADR-0005]）**：这里推测"冷启动读全量那笔跑不掉、IndexedDB 无优势"，
  > 实测推翻了——IndexedDB 冷启动读 5 万词反而更快（121ms vs 182ms），热路径写入也更快
  > （0ms vs 24ms），容量高三个数量级且不需要 `unlimitedStorage`。不引入的真正理由是它
  > 意味着第二次动用户真实词库的迁移，而现状尚未触及天花板——不是性能。触发条件见
  > [ADR-0005]。
