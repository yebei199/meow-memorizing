...
- **unlimitedStorage**：5 万词约 6.97MB，占 local 默认 10MB 上限的 70%，没有余量（将来
  若缓存 `definition` 字段会直接爆）。该权限会出现在商店审核与安装提示里。
...
- **IndexedDB**：容量与查询能力都更强，但 worker 冷启动仍要读全量建自动机（约 190ms 那笔
  跑不掉），当前瓶颈不在此，先不引入。
